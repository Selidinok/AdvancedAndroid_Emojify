...
class Emojifier {
    ...
    private static void whichEmoji(Face face) {
        ...
        boolean leftEyeOpen = face.getIsLeftEyeOpenProbability() > EYE_OPEN_PROB_THRESHOLD;
        boolean rightEyeOpen = face.getIsRightEyeOpenProbability() > EYE_OPEN_PROB_THRESHOLD;
        // TODO (5): Create an if/else system that selects the appropriate emoji based on the above booleans and log the result.
        Emoji emoji;
        if (smiling) {
            if (leftEyeOpen && rightEyeOpen) emoji = Emoji.OPEN_EYE_SMILE;
            else if (leftEyeOpen && !rightEyeOpen) emoji = Emoji.RIGHT_WINK;
            else if (rightEyeOpen && !leftEyeOpen) emoji = Emoji.LEFT_WINK;
            else emoji = Emoji.CLOSE_EYE_SMILE;
        } else {
            if (leftEyeOpen && rightEyeOpen) emoji = Emoji.FROWN;
            else if (leftEyeOpen && !rightEyeOpen) emoji = Emoji.RIGHT_WINK_FROWN;
            else if (rightEyeOpen && !leftEyeOpen) emoji = Emoji.LEFT_WINK_FROWN;
            else emoji = Emoji.ClOSE_EYE_FROWN;
        }

        Log.d(LOG_TAG, "whichEnoji: " + emoji.name());
    }
